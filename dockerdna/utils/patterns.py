...
SECRET_PATTERNS = [
    ...
    ("AWS Secret Access Key",     r"(?i)aws.{0,20}secret.{0,20}['\"][0-9a-zA-Z/+]{40}['\"]", "CRITICAL", "CIS-4.10"),
    ("Google API Key",            r"AIza[0-9A-Za-z\-_]{35}",                        "CRITICAL", "CIS-4.10"),
    ("Google OAuth Token",        r"ya29\.[0-9A-Za-z\-_]+",                         "HIGH",     "CIS-4.10"),
    ("Azure Connection String",   r"DefaultEndpointsProtocol=https;AccountName=",   "CRITICAL", "CIS-4.10"),
    ("Azure SAS Token",           r"sig=[A-Za-z0-9%+/=]{43,}",                      "CRITICAL", "CIS-4.10"),
    # Auth tokens
    ("Generic API Key",          r"(?i)(api_key|apikey|api-key)\s*[=:]\s*['\"]?[A-Za-z0-9_\-]{20,}['\"]?", "HIGH", "CIS-4.10"),
    ("Generic Secret",           r"(?i)(secret|password|passwd|pwd)\s*[=:]\s*['\"]?[A-Za-z0-9!@#$%^&*()_\-+=]{8,}['\"]?", "HIGH", "CIS-4.10"),
    ("Bearer Token",             r"[Bb]earer\s+[A-Za-z0-9\-_=]+\.[A-Za-z0-9\-_=]+\.[A-Za-z0-9\-_=]+", "HIGH", "CIS-4.10"),
    ("JWT Token",                r"eyJ[A-Za-z0-9\-_=]+\.[A-Za-z0-9\-_=]+\.[A-Za-z0-9\-_=]+", "HIGH", "CIS-4.10"),
    # Private keys
    ("RSA Private Key",          r"-----BEGIN RSA PRIVATE KEY-----",                "CRITICAL", "CIS-4.10"),
    ("EC Private Key",           r"-----BEGIN EC PRIVATE KEY-----",                 "CRITICAL", "CIS-4.10"),
    ("OpenSSH Private Key",      r"-----BEGIN OPENSSH PRIVATE KEY-----",            "CRITICAL", "CIS-4.10"),
    ...
    # Database URIs
    ("Database URI",             r"(?i)(mysql|postgresql|postgres|mongodb|redis|mssql)://[^@\s]+:[^@\s]+@", "CRITICAL", "CIS-4.10"),
    ("Connection String",        r"(?i)(Server|Data Source)=[^;]+;(User ID|uid)=[^;]+;(Password|pwd)=[^;]+", "CRITICAL", "CIS-4.10"),
    # Dockerfile-specific
    ("Hardcoded ENV Secret",     r"(?i)^ENV\s+(PASSWORD|SECRET|API_KEY|TOKEN|PRIVATE_KEY)\s*=\s*\S+", "CRITICAL", "CIS-4.9"),
    ("ARG Secret",               r"(?i)^ARG\s+(PASSWORD|SECRET|API_KEY|TOKEN|PRIVATE_KEY)\s*=\s*\S+", "HIGH", "CIS-4.9"),
]
...
CIS_RULES: list[CISRule] = [
    ...
    CISRule("CIS-4.1",  "Ensure a user for the container has been created",
            1, "HIGH",
    ...
    CISRule("CIS-4.2",  "Ensure container images do not use the latest tag",
            1, "MEDIUM",
    ...
    CISRule("CIS-4.3",  "Ensure unnecessary packages are not installed",
            1, "MEDIUM",
    ...
    CISRule("CIS-4.8",  "Ensure setuid/setgid bits are removed",
            2, "HIGH",
            "Binaries with setuid/setgid can allow privilege escalation.",
            "Add `RUN find / -perm /6000 -type f -exec chmod a-s {} +` to the Dockerfile.",
            "setuid_binaries"),

    CISRule("CIS-4.9",  "Ensure sensitive data is not stored in Dockerfile ENV/ARG",
            1, "CRITICAL",
    ...
    CISRule("CIS-4.10", "Ensure secrets are not hardcoded in image layers",
            1, "CRITICAL",
    ...
    CISRule("CIS-5.1",  "Ensure AppArmor Profile is applied",
            1, "MEDIUM",
...
UNNECESSARY_PACKAGES = [
    "curl", "wget", "nc", "netcat", "ncat", "nmap",
    "telnet", "ftp", "vim", "nano", "emacs",
    "gcc", "g++", "make", "gdb", "strace", "ltrace",
    "perl", "ruby", "python2", "python2.7",
